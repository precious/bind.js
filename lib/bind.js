            ...
            var oninput = function () {
              ...
              if (mirror && (element.nodeName === 'TEXTAREA' || (element.nodeName === 'INPUT' && (simpleInputTypes.indexOf(element.type) !== -1))) && typeof result === 'string') {
                if (this.value !== result) {
                  var _pos = this.selectionStart - (this.value.length - result.length), _fire = document.activeElement === this && this.selectionStart == this.selectionEnd;
                  this.value = result;
                  if (_fire) {
                    this.setSelectionRange(_pos, _pos);
                  }
                }
              }
