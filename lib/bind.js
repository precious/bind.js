...
var Bind = (function Bind(global) {
  ...
  function AugmentedArray(callback, settings) {
    ...
    this.replaceWith = function (another_array) {
      this.splice.apply(this, [0, this.length].concat(another_array));
    };
  ...
  function extend(target, object, settings, _path) {
    ...
    forEach(Object.getOwnPropertyNames(object), function eachProp(key) {
      ...
      if (typeof selector === 'function') {
        ...
        var simpleInputTypes = ['password', 'text', 'search', 'email', 'url', 'tel', 'number'];
            ...
            if (elements[0].type === 'checkbox') {
              ...
              for (var _i = 0, _length = elements.length; _i < _length; _i++) {
                if (elements[_i].checked) {
                  value.push(parse(elements[_i].value));
                }
              }
            } else {
        ...
        forEach(elements, function (element) {
          if (element.nodeName === 'INPUT' || element.nodeName === 'SELECT' || element.nodeName === 'TEXTAREA') {
            var oninput = function () {
              ...
              if (mirror && (element.nodeName === 'TEXTAREA' || (element.nodeName === 'INPUT' && (simpleInputTypes.indexOf(element.type) !== -1))) && typeof result === 'string') {
                if (this.value !== result) {
                  var self = this, _pos = this.selectionStart - (this.value.length - result.length), _fire = document.activeElement === this && this.selectionStart == this.selectionEnd;
                  this.value = result;
                  if (_fire) {
                    setTimeout(function () {  // workaround for chrome on android
                      self.setSelectionRange(_pos, _pos);
                    }, 0);
                  }
                }
              }
